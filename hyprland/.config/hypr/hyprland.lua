...
-- 2. Dynamic Workspace Naming & Nerd Font Glyph Icons
local app_glyphs = {
    ["google-chrome"] = "",
    ["beekeeper-studio"] = "",
    ["thunar"] = "",
...
local function update_workspace_names()
    local windows = hl.get_windows()
    
    -- Initialize glyph lookup for workspaces 1-10
    local ws_glyphs = {}
    for i = 1, 10 do
        ws_glyphs[i] = {}
    end
    
    -- Extract open window classes per workspace
    for _, win in ipairs(windows) do
        if win.workspace and win.workspace.id and win.class then
            local ws_id = win.workspace.id
            if ws_id >= 1 and ws_id <= 10 then
                local win_class = win.class:lower()
                local matched = false
                
                -- Check against our Nerd Font glyph map
                for pattern, glyph in pairs(app_glyphs) do
                    if string.find(win_class, pattern:lower(), 1, true) then
                        ws_glyphs[ws_id][glyph] = true
                        matched = true
                    end
                end
                
                -- Fallback to generic window glyph if no specific app matched
                if not matched then
                    ws_glyphs[ws_id][""] = true
                end
            end
        end
    end
    
    -- Rename workspaces based on active applications
    for ws_id = 1, 10 do
        local name_parts = {}
        for glyph, _ in pairs(ws_glyphs[ws_id]) do
            table.insert(name_parts, glyph)
        end
        
        local new_name
        if #name_parts > 0 then
            new_name = tostring(ws_id) .. ": " .. table.concat(name_parts, " ")
        else
            new_name = tostring(ws_id)
        end
        
        hl.dispatch(hl.dsp.workspace.rename({ workspace = tostring(ws_id), name = new_name }))
    end
end

-- Subscribe to window and workspace lifecycle events to update names dynamically
hl.on("window.open", update_workspace_names)
hl.on("window.close", update_workspace_names)
hl.on("window.destroy", update_workspace_names)
hl.on("window.move_to_workspace", update_workspace_names)
hl.on("workspace.created", update_workspace_names)
hl.on("workspace.removed", update_workspace_names)

-- Run initial workspace naming once configuration is loaded
